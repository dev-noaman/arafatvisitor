"use strict";
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __spreadArray = (this && this.__spreadArray) || function (to, from, pack) {
    if (pack || arguments.length === 2) for (var i = 0, l = from.length, ar; i < l; i++) {
        if (ar || !(i in from)) {
            if (!ar) ar = Array.prototype.slice.call(from, 0, i);
            ar[i] = from[i];
        }
    }
    return to.concat(ar || Array.prototype.slice.call(from));
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.createMockRequest = exports.createMockJwtPayload = void 0;
exports.createTestingModule = createTestingModule;
exports.createTestingModuleForService = createTestingModuleForService;
exports.createTestingModuleForController = createTestingModuleForController;
exports.createMockService = createMockService;
var testing_1 = require("@nestjs/testing");
var prisma_service_1 = require("../prisma/prisma.service");
var prisma_mock_1 = require("./prisma.mock");
/**
 * NestJS Module Factory Helpers
 *
 * Provides utility functions to create test modules with mocked dependencies.
 * This simplifies setting up tests for NestJS services and controllers.
 *
 * @example
 * ```typescript
 * import { createTestingModule } from '@/test-utils/module.factory'
 *
 * describe('AuthService', () => {
 *   let service: AuthService
 *   let module: TestingModule
 *
 *   beforeEach(async () => {
 *     module = await createTestingModule({
 *       providers: [AuthService],
 *     })
 *     service = module.get(AuthService)
 *   })
 *
 *   afterEach(async () => {
 *     await module.close()
 *   })
 * })
 * ```
 */
/**
 * Creates a testing module with mocked Prisma service
 *
 * @param options - Module builder options
 * @returns TestingModule instance
 */
function createTestingModule() {
    return __awaiter(this, arguments, void 0, function (options) {
        if (options === void 0) { options = {}; }
        return __generator(this, function (_a) {
            return [2 /*return*/, testing_1.Test.createTestingModule(__assign(__assign({}, options), { providers: __spreadArray(__spreadArray([], (options.providers || []), true), [
                        // Always provide the mocked Prisma service
                        {
                            provide: prisma_service_1.PrismaService,
                            useValue: prisma_mock_1.prismaMock,
                        },
                    ], false) })).compile()];
        });
    });
}
/**
 * Creates a testing module with a specific service and its dependencies
 *
 * @param serviceClass - The service class to test
 * @param additionalProviders - Additional providers to include
 * @returns TestingModule instance with the service
 */
function createTestingModuleForService(serviceClass_1) {
    return __awaiter(this, arguments, void 0, function (serviceClass, additionalProviders) {
        if (additionalProviders === void 0) { additionalProviders = []; }
        return __generator(this, function (_a) {
            return [2 /*return*/, testing_1.Test.createTestingModule({
                    providers: __spreadArray(__spreadArray([
                        serviceClass
                    ], additionalProviders, true), [
                        {
                            provide: prisma_service_1.PrismaService,
                            useValue: prisma_mock_1.prismaMock,
                        },
                    ], false),
                }).compile()];
        });
    });
}
/**
 * Creates a testing module for a controller with mocked dependencies
 *
 * @param controllerClass - The controller class to test
 * @param providers - Service providers to include (will be mocked)
 * @returns TestingModule instance with the controller
 */
function createTestingModuleForController(controllerClass_1) {
    return __awaiter(this, arguments, void 0, function (controllerClass, providers) {
        var mockedProviders;
        if (providers === void 0) { providers = []; }
        return __generator(this, function (_a) {
            mockedProviders = providers.map(function (provider) { return ({
                provide: provider,
                useValue: {
                // Add common mock methods here
                // Specific mocks should be set in individual tests
                },
            }); });
            return [2 /*return*/, testing_1.Test.createTestingModule({
                    controllers: [controllerClass],
                    providers: __spreadArray(__spreadArray([], mockedProviders, true), [
                        {
                            provide: prisma_service_1.PrismaService,
                            useValue: prisma_mock_1.prismaMock,
                        },
                    ], false),
                }).compile()];
        });
    });
}
/**
 * Helper to create a mock service with common methods
 *
 * @example
 * ```typescript
 * const mockAuthService = createMockService<AuthService>({
 *   validateUser: jest.fn().mockResolvedValue(user),
 *   login: jest.fn().mockResolvedValue(token),
 * })
 * ```
 */
function createMockService(methods) {
    return __assign({}, methods);
}
/**
 * Helper to create a mock JWT payload
 */
var createMockJwtPayload = function (overrides) {
    if (overrides === void 0) { overrides = {}; }
    return (__assign({ sub: 1, email: 'test@example.com', role: 'USER', iat: Date.now(), exp: Date.now() + 3600 }, overrides));
};
exports.createMockJwtPayload = createMockJwtPayload;
/**
 * Helper to create a mock request with user
 */
var createMockRequest = function (user) {
    if (user === void 0) { user = (0, exports.createMockJwtPayload)(); }
    return ({
        user: user,
        headers: {},
        query: {},
        params: {},
        body: {},
    });
};
exports.createMockRequest = createMockRequest;
