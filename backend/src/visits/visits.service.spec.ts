...
describe("VisitsService", () => {
  ...
  describe("preRegister", () => {
    ...
    it("should throw ForbiddenException when user is not a host", async () => {
      ...
      // Act & Assert
      await expect(service.preRegister(preRegisterDto, 1)).rejects.toThrow(
        ForbiddenException,
      );
      await expect(service.preRegister(preRegisterDto, 1)).rejects.toThrow(
  ...
  describe("approve", () => {
    ...
    it("should throw NotFoundException when visit not found", async () => {
      ...
      // Act & Assert
      await expect(service.approve("visit-999", 1)).rejects.toThrow(
        NotFoundException,
      );
      await expect(service.approve("visit-999", 1)).rejects.toThrow(
        "Visit not found",
    ...
    it("should throw ForbiddenException when not host of visit", async () => {
      ...
      // Act & Assert
      await expect(service.approve("visit-1", 2)).rejects.toThrow(
        ForbiddenException,
      );
      await expect(service.approve("visit-1", 2)).rejects.toThrow(
        "Not your visit",
    ...
    it("should throw BadRequestException when visit is not pending", async () => {
      ...
      // Act & Assert
      await expect(service.approve("visit-1", 1)).rejects.toThrow(
        BadRequestException,
      );
      await expect(service.approve("visit-1", 1)).rejects.toThrow(
        "Visit is not pending approval",
      );
      expect(prismaMock.visit.update).not.toHaveBeenCalled();
    });
  });
  ...
  describe("findBySessionId", () => {
    ...
    it("should throw NotFoundException when visit not found", async () => {
      ...
      // Act & Assert
      await expect(service.findBySessionId("nonexistent")).rejects.toThrow(
        NotFoundException,
      );
      await expect(service.findBySessionId("nonexistent")).rejects.toThrow(
        "Visit not found",
  ...
  describe("checkout", () => {
    ...
    it("should throw BadRequestException when visitor already checked out", async () => {
      ...
      // Act & Assert
      await expect(service.checkout("session-123", 1)).rejects.toThrow(
        BadRequestException,
      );
      await expect(service.checkout("session-123", 1)).rejects.toThrow(
    ...
    it("should throw NotFoundException when visit not found", async () => {
      ...
      // Act & Assert
      await expect(service.checkout("nonexistent", 1)).rejects.toThrow(
        NotFoundException,
      );
      await expect(service.checkout("nonexistent", 1)).rejects.toThrow(
        "Visit not found",
