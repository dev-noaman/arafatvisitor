...
var AuditInterceptor = function () {
    ...
    var AuditInterceptor = _classThis = /** @class */ (function () {
        ...
        AuditInterceptor_1.prototype.intercept = function (context, next) {
            ...
            var ip = request.ip || ((_a = request.socket) === null || _a === void 0 ? void 0 : _a.remoteAddress);
            var userAgent = request.get('user-agent') || undefined;
            return next.handle().pipe((0, operators_1.tap)({
                next: function () {
                    if (['POST', 'PUT', 'PATCH', 'DELETE'].includes(method)) {
                        var url = request.url;
                        var entity = 'Unknown';
                        var action = "".concat(method.toLowerCase());
                        ...
                        _this.auditService.log({
                            userId: userId,
                            action: "".concat(entity, ".").concat(action),
                            entity: entity,
                            ip: ip,
                            userAgent: userAgent,
                        }).catch(function () { });
